...
from simulation import simulate, experiment, analyze, display_params, display_tree, display_stats
...
def main(sample_size: int, sample_size_end: int, sample_size_step: int, mu: float, mu_thold: float, num_iter: int,
    ...
    if test:
        ...
        # single simulation
        simulate(MODELS, num_iter, sample_size, mu, data, tree_dir=tree_dir, exp=False, graphics=graphics, verbose=verbose)
...
if __name__ == '__main__':
    ...
    parser.add_argument("-n", "--sample_size", nargs='?', default=15, type=int,
                        help="init population sample size, subject to change in experiment. "
                             "The value is final for testing")
    parser.add_argument("-e", "--sample_size_end", nargs='?', default=25, type=int,
                        help="end of sample size range for experiment")
    parser.add_argument("-s", "--sample_size_step", nargs='?', default=3, type=int,
                        help="sample size range step unit for experiment")

    parser.add_argument("-m", "--mu", nargs='?', default=0.5, type=float,
    ...
    parser.add_argument("-t", "--num_tests", nargs='?', default=1, type=int,
                        help="number of processes for experiment (only for experiment mode)")
    ...
    #                     help="path to the output directory for simulated models")
    # parser.add_argument("--save_model", action="store_true",
    #                     help="saves serialized models for each combinations of sample size and mu")
    ...
    main(sample_size=args.sample_size, sample_size_end=args.sample_size_end, sample_size_step=args.sample_size_step,
         ...
         tree_dir=args.tree_dir, test=args.test, graphics=args.graphics, verbose=args.verbose)
